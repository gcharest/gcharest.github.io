...
4. À la fin de l'installation, un icône VirtualBox devrait maintenant être disponible dans votre dossier **Applications** et un message devrait s'afficher vous demandant de redémarrer votre Mac.
...
1. Si vous utilisez un Mac avec un écran à très haute résolution, du type Retina Display par exemple, les icônes et le texte risquent d'être très petits et difficiles à lire.
   1. Pour les ajuster, il est possible d'utiliser la fonction **Make everything bigger** de Windows 10.
    Pour lancer ce menu, vous pouvez cliquer dans l'outil de recherche située en bas à gauche et commencer à taper **Make everything bigger**, ce qui devrait faire afficher les résultats de recherche et vous devriez voir un résultat dans **Best match** concordant avec le texte entré.
...
1. Lorsque votre machine virtuelle sera bien configurée et que les performances seront acceptables, il sera temps d'installer les outils fournis par VirtualBox pour améliorer les fonctionnalités.
   ...
   Double-cliquez et la section de droite devrait maintenant afficher le contenu du disque virtuel ajouté lors de la configuration de la machine virtuelle.
